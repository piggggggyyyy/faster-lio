...
class ImuProcess {
    ...
    void Process(const common::MeasureGroup &meas,  std::shared_ptr<InvariantKF::invkf> kf_state,PointCloudType::Ptr pcl_un_);
    void Process(const common::MeasureGroup &meas, esekfom::esekf<state_ikfom, 12, input_ikfom> &kf_state,
                 PointCloudType::Ptr pcl_un_);//todo

    std::ofstream fout_imu_;
    Eigen::Matrix<double, 12, 12> Q_;
    Eigen::Matrix<double, 24, 24> Q_inv;
    common::V3D cov_acc_;
    common::V3D cov_gyr_;
   ...
   private:
    ...
    void IMUInit(const common::MeasureGroup &meas, std::shared_ptr<InvariantKF::invkf> kf_statekf_state, int &N);//todo
    void UndistortPcl(const common::MeasureGroup &meas, std::shared_ptr<InvariantKF::invkf> kf_statekf_state,//todo
                      PointCloudType &pcl_out);
    void IMUInit(const common::MeasureGroup &meas, esekfom::esekf<state_ikfom, 12, input_ikfom> &kf_state, int &N);//todo
    void UndistortPcl(const common::MeasureGroup &meas, esekfom::esekf<state_ikfom, 12, input_ikfom> &kf_state,//todo
                      PointCloudType &pcl_out);
...
void ImuProcess::UndistortPcl(const common::MeasureGroup &meas, std::shared_ptr<InvariantKF::invkf> kf_state,
                              PointCloudType &pcl_out){
 /*** add the imu_ of the last frame-tail to the of current frame-head ***/
    auto v_imu = meas.imu_;
    v_imu.push_front(last_imu_);
    ...
    /*** Initialize IMU pose ***/
    //state_ikfom imu_state = kf_state.get_x();
    InvariantKF::invkf::State24 state = kf_state->getX();
    IMUpose_.clear();
    IMUpose_.push_back(common::set_pose6d(0.0, acc_s_last_, angvel_last_, state.imu_state.x[0], state.imu_state.x[1],
                                          state.imu_state.R.asMatrix()));
    ...
    for (auto it_imu = v_imu.begin(); it_imu < (v_imu.end() - 1); it_imu++) {
        ...
        /* save the poses at each IMU measurements */
        state = kf_state->getX();
        angvel_last_ = angvel_avr - state.bg;
        acc_s_last_ = state.imu_state.R * (acc_avr - state.ba);
        for (int i = 0; i < 3; i++) {
            acc_s_last_[i] += state.gravity[i];
        }

        double &&offs_t = tail->header.stamp.toSec() - pcl_beg_time;
        IMUpose_.emplace_back(common::set_pose6d(offs_t, acc_s_last_, angvel_last_, state.imu_state.x[0], state.imu_state.x[1],
                                          state.imu_state.R.asMatrix()));
    }
    ...
    for (auto it_kp = IMUpose_.end() - 1; it_kp != IMUpose_.begin(); it_kp--) {
        ...
        for (; it_pcl->curvature / double(1000) > head->offset_time; it_pcl--) {
            ...
            common::V3D P_i(it_pcl->x, it_pcl->y, it_pcl->z);
            common::V3D T_ei(pos_imu + vel_imu * dt + 0.5 * acc_imu * dt * dt - state.imu_state.x[1]);
            common::V3D p_compensate =
                state.ext_r.conjugate() *
                (state.imu_state.R.asMatrix().conjugate() * (R_i * (state.ext_r * P_i + state.ext_t) + T_ei) -
                 state.ext_t);  // not accurate!

            // save Undistorted points and their rotation
            it_pcl->x = p_compensate(0);
            it_pcl->y = p_compensate(1);
            it_pcl->z = p_compensate(2);
            //LOG(INFO) << "test" << it_pcl->x << "," << it_pcl->y << ","<< it_pcl->z ;
            if (it_pcl == pcl_out.points.begin()) {
                break;
            }
        }
...
void ImuProcess::IMUInit(const common::MeasureGroup &meas, std::shared_ptr<InvariantKF::invkf> kf_state,
                         int &N){
    common::V3D cur_acc, cur_gyr;
...
void ImuProcess::Process(const common::MeasureGroup &meas, std::shared_ptr<InvariantKF::invkf> kf_state, PointCloudType::Ptr cur_pcl_un_){
    if (meas.imu_.empty()) {
        return;
    }
    ROS_ASSERT(meas.lidar_ != nullptr);
