    ...
    Timer::Evaluate(
        [&, this]() {
            ...
            std::for_each(std::execution::par_unseq, index.begin(), index.end(), [&](const size_t &i) {
                ...
                common::V3F point = R * point_this;
                // /*** get the normal vector of closest surface/corner ***/
                common::V3F norm_vec = corr_norm_[i].head<3>();
                common::V3F A(norm_vec.transpose() * (-1) * SKEW_SYM_MATRIX(point));
...
void LaserMapping::SetPosestamp(T &out) {
    Eigen::Vector3d pos = state.imu_state.x[1];
    Eigen::Quaterniond rot = state.imu_state.R.asQuaternion();
...
void LaserMapping::PointBodyToWorld(const PointType *pi, PointType *const po) {
    ...
    //state here is not normal ,debug is needed
    common::V3D p_global(state.imu_state.R.asQuaternion() * (state.ext_r * p_body + state.ext_t) +
                         state.imu_state.x[1]);
...
void LaserMapping::PointBodyLidarToIMU(PointType const *const pi, PointType *const po) {
    ...
    common::V3D p_body_imu(state.ext_r * p_body_lidar + state.ext_t);
